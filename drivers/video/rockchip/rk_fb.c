...
#ifdef CONFIG_OLEGK0_LINUX_XVIDEO
#ifdef CONFIG_MALI
#include "ump/ump_kernel_interface.h"
#endif
#endif //CONFIG_OLEGK0_LINUX_XVIDEO
...
int rk_fb_switch_screen(rk_screen *screen ,int enable ,int lcdc_id)
{
	...
	var = &info->var;
	u16 x_aspect_ratio_correction = dev_drv->screen->x_res - dev_drv->screen->y_res*var->xres_virtual/var->yres_virtual;
	xpos = (x_aspect_ratio_correction + dev_drv->screen->x_res - dev_drv->screen->x_res*dev_drv->x_scale/100)>>1;
	ypos = (dev_drv->screen->y_res - dev_drv->screen->y_res*dev_drv->y_scale/100)>>1;
	xsize = dev_drv->screen->x_res * dev_drv->x_scale/100 - x_aspect_ratio_correction;
...
int rk_fb_disp_scale(u8 scale_x, u8 scale_y,u8 lcdc_id)
{
	...
	u16 x_aspect_ratio_correction = screen_x - screen_y*var->xres_virtual/var->yres_virtual;
	xpos = (x_aspect_ratio_correction + screen_x-screen_x*scale_x/100)>>1;
	ypos = (screen_y-screen_y*scale_y/100)>>1;
	xsize = screen_x*scale_x/100 - x_aspect_ratio_correction;
...
#ifndef CONFIG_OLEGK0_LINUX_XVIDEO
