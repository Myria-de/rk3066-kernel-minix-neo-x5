...
#define DEBUG
#ifdef DEBUG
#define MODEMDBG(x...) printk("<--MT6229-DEBUG--> "x)
#else
...
static void do_wakeup(struct work_struct *work)
{
      if(suspend_int)
         {
             gpio_set_value(gpdata->ap_wakeup_bp, GPIO_HIGH);//gpio_set_value(gpdata->ap_wakeup_bp, 0);
             suspend_int = 0;
         }
...
int modem_poweron_off(int on_off)
{
  ...
  if(on_off)
  {
		...
		msleep(10);
		gpio_set_value(pdata->bp_power, GPIO_LOW);
		gpio_set_value(pdata->ap_wakeup_bp, GPIO_HIGH);//GPIO_LOW
  }
  else
  {
		MODEMDBG("modem_poweroff\n");
		gpio_set_value(pdata->bp_power, GPIO_LOW);//GPIO_HIGH
		gpio_set_value(pdata->ap_wakeup_bp, GPIO_LOW);//GPIO_HIGH
  }
  return 0;
}
...
#endif
{
   ...
   if (new_state == 1){
     printk("MT6229: %s, c(%d), open modem \n", __FUNCTION__, new_state);
	 modem_poweron_off(1);
   }else if(new_state == 0){
     printk("MT6229: %s, c(%d), close modem \n", __FUNCTION__, new_state);
	 modem_poweron_off(0);
   }else{
     printk("MT6229: %s, invalid parameter \n", __FUNCTION__);
   }
...
{
        printk("MT6229: *********************mt6229____suspend\n");
...
static void rk29_early_resume(struct early_suspend *h)
{
	 if(suspend_int)
	{
         printk("MT6229: ***************mt6229____resume\n");
	gpio_set_value(gpdata->ap_wakeup_bp, GPIO_HIGH);//gpio_set_value(gpdata->ap_wakeup_bp, 0);
	 suspend_int = 0;
 	}
}
...
static int mt6229_probe(struct platform_device *pdev)
{
	...
	if(mt6229_data == NULL)
	{
		printk("MT6229: failed to request mt6229_data\n");
		goto err2;
	}
	platform_set_drvdata(pdev, mt6229_data);		
	result = gpio_request(pdata->ap_wakeup_bp, "mt6229");
	if (result) {
		printk("MT6229: failed to request AP_BP_WAKEUP gpio\n");
		goto err1;
	}	
	...
	if(irq < 0)
	{
		...
		printk("MT6229: failed to request bp_wakeup_ap\n");
	}
	result = gpio_request(pdata->bp_wakeup_ap, "bp_wakeup_ap");
	if (result < 0) {
		printk("MT6229: %s: gpio_request(%d) failed\n", __func__, pdata->bp_wakeup_ap);
	}
	...
	if (result < 0) {
		printk("MT6229: %s: request_irq(%d) failed\n", __func__, irq);
		gpio_free(pdata->bp_wakeup_ap);
		goto err0;
	}
	...
	if(result)
	{
		printk("MT6229: misc_register err\n");
	}	
...
int mt6229_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	do_wakeup_irq = 1;
	MODEMDBG("%s\n",__FUNCTION__);
	ap_wakeup_bp(pdev, 0);//ap_wakeup_bp(pdev, 1);
...
static int __init mt6229_init(void)
{
	...
	if (ret)
	{
		printk("MT6229: Fail to class rk291x_modem.\n");
	}
