...
#define BATT_MAX_VOL_VALUE                             4160               	//满电时的电池电压	 
#define BATT_ZERO_VOL_VALUE                            3490              	//关机时的电池电压
#define BATT_NOMAL_VOL_VALUE                         3800               

//定义ADC采样分压电阻，以实际值为准，单位K

#define BAT_PULL_UP_R                                         210

#define BAT_PULL_DOWN_R                                   190
...
{
#ifdef CONFIG_BATTERY_RK30_ADC_LINUX_OS_INTERFACE
	return -1;
#else
	char value[4];
	int* p = (int *)value;
...
#ifdef CONFIG_BATTERY_RK30_ADC_LINUX_OS_INTERFACE
	else if (gBatteryData->bat_status != POWER_SUPPLY_STATUS_DISCHARGING){
#else
...
#ifdef CONFIG_BATTERY_RK30_ADC_LINUX_OS_INTERFACE
static char *power_ac_supplied_to[] = {
	"battery",
};
#endif
...
#ifdef CONFIG_BATTERY_RK30_ADC_LINUX_OS_INTERFACE
	.supplied_to = power_ac_supplied_to,
	.num_supplicants = ARRAY_SIZE(power_ac_supplied_to),
#endif
...
{
	return (bat->bat_voltage < BATT_ZERO_VOL_VALUE) ? 0 : 1;
...
{		
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
			if (gBatteryData->bat_capacity > 98) val->intval = POWER_SUPPLY_CAPACITY_LEVEL_FULL;
			else if (gBatteryData->bat_capacity > 80) val->intval = POWER_SUPPLY_CAPACITY_LEVEL_HIGH;
			else if (gBatteryData->bat_capacity > 30) val->intval = POWER_SUPPLY_CAPACITY_LEVEL_NORMAL;
			else if (gBatteryData->bat_capacity > 7) val->intval = POWER_SUPPLY_CAPACITY_LEVEL_LOW;
