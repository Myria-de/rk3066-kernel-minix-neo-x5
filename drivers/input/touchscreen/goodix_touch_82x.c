...
#ifdef CONFIG_GT82X_LINUX_OS_SUPPORT // only 1 for Linux OS
#ifdef MAX_FINGER_NUM
	#undef MAX_FINGER_NUM
	#define MAX_FINGER_NUM 1
#endif
#endif //CONFIG_GT82X_LINUX_OS_SUPPORT
...
static void open_longkey_timer_82x(unsigned long data)
{
    ...
    add_timer(&longkey_timer_82x);
		printk("GT82X: Origin2LockPressCnt = %d\n",Origin2LockPressCnt); 
		printk("GT82X: lockflag = %d\n",lockflag);
}
#endif
...
#ifdef CONFIG_GT82X_LINUX_OS_SUPPORT
...
#ifdef CONFIG_GT82X_LINUX_OS_SUPPORT
		input_report_abs(ts->input_dev, ABS_X, X_value);
		input_report_abs(ts->input_dev, ABS_Y, Y_value);	
#else
...
//    ts->input_dev->keybit[BIT_WORD(BTN_TOUCH)] = BIT_MASK(BTN_TOUCH);
 //   ts->input_dev->absbit[0] = BIT(ABS_X) | BIT(ABS_Y) | BIT(ABS_PRESSURE);// absolute coor (x,y)
    ...
    ts->input_dev->keybit[BIT_WORD(BTN_LEFT)] = BIT_MASK(BTN_LEFT);
...
#ifndef CONFIG_GT82X_LINUX_OS_SUPPORT
...
static s32 goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("GT82X: goodix_ts_probe gt82x \n");
...
{
	int ret; 
		printk("GT82X: Start initialization.\n");
	...
	if (ret)
	{
	    printk("GT82X: Register raydium_ts driver failed gt82x.\n");
    		return ret;   //w++
	}
#ifdef LONGPRESS_LOCK_SPECKEY
	if(err_gt82x>=0)   //w++
